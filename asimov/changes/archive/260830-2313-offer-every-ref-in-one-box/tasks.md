...
- [x] 4_2 Round-2 fixes: the create-new submit route, the scrolled active row, and an answer that knows its opening — verified: pnpm exec vitest run 'src/webview/worktree/WorktreeCreateDialog.test.ts' && pnpm run check-types && pnpm run test:unit exit 0
  ...
  - **Plan**:
    ...
    8. Round-3 W4: `ready()` never opens, so the two cleanup regressions delivered under a token nothing had minted and were dropped before storing — each then passed on an empty map. They open first and assert the entry exists before asserting it is gone.
    9. Round-3 S3: the popup ceiling is measured rather than calculated. `calc(100vh - 100%)` resolves against the FIELD's height, not its position, so `openList` publishes `--wt-branch-room` from the input's own rect and the stylesheet consumes it.
