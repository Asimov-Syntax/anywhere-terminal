...
Review cycle 2, round 4 (verification): BLOCK — B4 confirmed fixed, B5 persists, and three new WARNs. Round-4 B5 is the sharper half of the same finding and it is right: `busyAgents` is counted from the debounced projection while `input.panes` is the live snapshot, so corroborating a claim with "the pane exists and has not exited" let a pane running RIGHT NOW pair with its own stale idle row and erase the registry record that would have refused. Fixed in task 6_1 — suppression requires `activity === "idle"`, while the pane list the report NAMES still excludes only `exited`, because those are different questions. Treated as remediation rather than a handback: D6 already decides that a claim we cannot corroborate is not a claim, the enumerated clause contradicted its own principle, and correcting it mints no invariant owner and leaves every rejected alternative rejected. D6's wording corrected in the same task.

W2 and W4 fixed in 6_1. W2's second half is the one that mattered: an enumeration that runs long while yielding NOTHING never enters the loop body, so no deadline check ran and the walk reported a measured zero — the strongest claim it can make — after blowing its budget. W4 was verified rather than taken on report: `defaultWait` in `sessionPreviewService.ts` was the same unref'd primitive, and W2 is what two owners of deadline semantics produces. Both now use `src/worktree/deadline.ts`.
