...
- 1_2 is a `manual` Verify no harness here can run — jsdom measures nothing and `@vscode/test-electron`
  cannot reach inside the webview iframe. The user walked the panel at both widths, with a previewed
  row, a preview-less row and an external row, and confirmed it; that walk is the tick's evidence.
