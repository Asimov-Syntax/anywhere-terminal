...
- [ ] 1_1 Derive activity confidence from the row's own clock
  - **Deps**: none
  ...
  - **Plan**:
    ...
    3. Thread the new clock argument through every existing caller in the same task — `src/webview/worktree/WorktreeView.ts` and `src/webview/worktree/WorktreeRemoveDialog.ts` — so the wave compiles on its own. Shapes, labels and hints are 1_2's.
...
- [ ] 1_2 Give the unconfirmed claim a static shape and a hint that cannot go stale
  ...
  - **Plan**:
    1. In `src/webview/worktree/worktreePanel.css`, add the unconfirmed shape — static, and distinct from both the animated running shape and every other state once motion is removed.
    ...
    3. Extend the shape guard's state list, which is keyed by the presented vocabulary, so the new member is covered rather than merely admitted.
