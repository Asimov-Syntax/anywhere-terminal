...
- [ ] 1_2 The refs reach the dialog on their own message
  - **Deps**: none
  - **Refs**: design.md D1
  - **Acceptance**:
  ...
  - **Plan**:
    ...
    2. `src/webview/worktree/WorktreeController.ts` requests them when the create dialog opens and routes the answer through a bound applier, exactly as `bindProvisioning` already does.
