...
- [ ] 1_2 A worktree is selectable, full stop
  - **Deps**: 1_4
  - **Refs**: specs/worktree-panel/spec.md#{a-worktree-can-be-selected-and-selection-is-an-explicit-act, selecting-a-worktree-opens-an-inspector-under-the-tree} <!-- design.md D2, D5 -->
  - **Acceptance**:
    - Outcome: activating a worktree row selects it and opens the inspector with no setting consulted
  ...
  - **Plan**:
    ...
    5. `WorktreeView.test.ts`'s `mount` omits `workbench`, which today means OFF, so five cases in that suite assert the OFF arm's "expansion equals card" without saying so. Move each to the ON arm's separate `.wt-group` wrapper and selected `.wt-card` — the expanded-worktree wrapper, collapse-and-restore, restored disclosure levels, empty persisted collapse list, and first-run collapse seeding. Read them one at a time: a collapsed selected worktree keeps `.wt-card` on purpose (design.md D5).

- [ ] 1_3 Scoping is always offered
  - **Deps**: 1_2
  - **Refs**: specs/tab-bar-component/spec.md#{scoping-is-offered-only-where-it-has-been-turned-on} <!-- design.md D2, D4, D5 -->
  - **Acceptance**:
    - Outcome: a persisted scope takes effect and the chip renders with no setting consulted, and the persisted key is unchanged
    - Verify: unit src/webview/tabBarScope.test.ts
  - **Plan**:
    1. In `src/webview/tabBarScope.ts` delete the `workbench` dep, the held flag and its runtime setter, and the branches that made the effective scope, the presence subscription and the dropped-scope notice inert.
    2. In `src/webview/tabBarScopeWiring.ts` delete `workbench` from the wiring deps and `setWorkbench` from the returned interface and its implementation.
    3. In `src/webview/main.ts` delete the `workbench` property from the `wireTabBarScope` literal and, now that its body has nothing left, the whole `onWorktreeWorkbench` handler — the router's member is optional, so this type-checks before 2_1 removes it.
    4. Leave the persisted per-surface scope key untouched, along with everything that reads or writes it (design.md D4).
    5. Delete the OFF-arm and transition cases in `src/webview/tabBarScope.test.ts` and `src/webview/tabBarScopeWiring.test.ts`.
...
- [ ] 2_1 Stop carrying the flag to the webview
  ...
  - **Plan**:
    ...
    2. Delete the OFF fixture value from `src/webview/messaging/MessageRouter.test.ts` and `src/extension.worktreeAssembly.test.ts`.
