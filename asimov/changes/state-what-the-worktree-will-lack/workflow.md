...
Deferred (2_4, review W4): each adapter mints item ids from its own counter, so WT-012.3's three adapters merging under WT-012.4 will collide. Not fixed here — minting in an assembly layer that does not exist, or a per-adapter prefix guessing the merge's shape, is the seam-from-one-example design.md D2 rejects. The obligation is written onto `ProvisionItemId` so the merge task inherits it at the point it will read.
