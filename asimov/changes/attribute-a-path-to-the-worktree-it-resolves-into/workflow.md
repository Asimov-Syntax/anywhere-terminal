...
- Round-4 build: both blockers were transitions where the host declined to call the reconciliation B6 had already fixed — a null root treated as nothing to do rather than as the empty set, and an `attach` that installed the folder listener without reconciling the root it was attaching to. The chair was right that deleting the host root-transition test in 4_1 is what left them uncovered: I dropped it because the version I wrote called a method the host does not have, when the correct move was to rewrite it against the attach harness. Both transitions are now covered there, and both mutations die.
- Cycle 2 exhausted at round 5 with B11 open (a resolved-root correction posted inside the init retry window reaches a webview with no controller and is dropped; the retry then delivers the payload captured before the resolution). Confirmed against code. Thrash stop reached — the decision between a designed fix, risk acceptance, and one bounded extension round is the user's, and two of the three are never auto-chosen under fastlane.
