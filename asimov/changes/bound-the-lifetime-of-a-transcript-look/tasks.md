...
- [x] 1_2 Keep the attempt registry outside the cache so eviction releases nothing — verified: pnpm exec vitest run 'src/worktree/sessionPreviewService.test.ts' && pnpm run check-types && pnpm run test:unit exit 0
  - **Deps**: 1_1
  ...
  - **Acceptance**:
    ...
    - Verify: unit src/worktree/sessionPreviewService.test.ts
  - **Plan**:
    1. In `src/worktree/sessionPreviewService.ts`, add a service-scoped `const outstanding = new Map<string, Held>()` beside `held`, holding the entry that owns each unsettled look.
