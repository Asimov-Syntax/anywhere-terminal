...
- [x] 4_2 Round-1 fixes: B2, B3(a), W1 — one scan, two views, and a partial scan that says so — verified: pnpm exec vitest run 'src/worktree/worktreeBlockers.test.ts' && pnpm run check-types && pnpm run test:unit exit 0
  ...
  - **Acceptance**:
    - Outcome: A session whose canonical record is rooted outside the target does not refuse its removal
    - Verify: unit src/worktree/worktreeBlockers.test.ts
  - **Plan**:
    1. `src/vault/readers/runningSessions.ts` exports the canonical derivation as a pure function over records already in hand — the SAME `winsDedupe` the live reader runs, lifted rather than copied, so the selection rule keeps one home. `listRunningClaudeSessions` calls it where it inlines the rule today.
