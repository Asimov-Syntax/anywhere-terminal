...
The decoder correlates bounded root-reachable Cursor `Task`/`Agent` `tool-call` and `tool-result` blocks by `toolCallId`; blocking results attach to the invocation, while background launch results contribute only a safe task identity and later injected completion notices attach the final result. It recognizes a child Agent ID from the invocation's own bounded `resume` argument and from those correlated bounded results, using a strict safe-id shape; unrelated tool output is never scanned for child identity.

Cursor addresses one sub-agent through many invocations: a launch call declares `subagent_type`, and each continuation carries only `resume: <agent-id>` plus its own description and prompt. Keying cards by `toolCallId` alone therefore splits one agent into per-call cards and labels continuations with the invoking tool's name. Invocations are merged by agent identity after normalization — in a shared pass, because the JSONL mirror reuses the normalizer without the store's correlation maps — collapsing them at the first invocation's position with the launch call's declared type, description, and prompt, and the newest correlated result and status. Sub-agent stats count merged agents. `args.resume` is preferred over parsing `Agent ID:` from result text: continuation results do not always repeat that line.

A child Agent ID is not a CLI Resume identity. After parent CLI detail validates the parent store and cwd, the combined reader derives that cwd's exact project bucket once per detail and point-resolves only `<bucket>/agent-transcripts/<child-id>/<child-id>.jsonl` or the legacy flat equivalent per child. Exactly one contained candidate converts the merged invocation into the existing lazy `subagentSession` shape. Expanding the existing `AGENT` card therefore reuses nested-detail IPC and timeline rendering for the saved child transcript, now covering every turn of that agent; no match, ambiguity, or limited child detail retains the bounded inline Prompt/Result card. No global child-id scan or fabricated transcript is permitted.
...
Project JSONL envelopes expose no metadata-level parent/child marker: observed parent and child files share the same `role` plus `message.content` shape. Listing every unmatched JSONL would therefore publish child agents as top-level sessions, while classifying them would require transcript-content scans during indexing. The accepted boundary lists neither: project JSONL is an exact detail source only. A same-project id matching a validated CLI chat supplies mirror/fallback detail; a safe agent identity from a validated parent Task invocation or result supplies lazy child detail. Orphan unmatched JSONL remains hidden rather than weakening the metadata-only list contract.

Hiding a row is not an access boundary. A readable `cursor:project:<bucket>:<id>` on the wire remains decodable by any detail request that knows the pair, including an orphan the list deliberately hides. Child detail is therefore reached only through a host-issued opaque locator: when parent detail emits a child stub, the host registers it in a bounded LRU (256 entries) mapping locator → resolved candidate, and `getDetail`/`getEntry` serve a `cursor:child:<token>` only from that registry. The token is derived from `(parent session id, child agent id, per-process salt)` so it stays stable while the parent detail is re-read under live follow — an expanded card survives refresh — yet is not derivable outside the process. The CLI mirror path never crosses IPC and stays an internal host-side lookup.
...
Resume and Copy Resume Command share one host-side proof before executable resolution or side effects. The proof reads only the bounded supported store profile plus `meta['0']` from one WAL-aware disposable snapshot and requires stored `agentId === candidate.chatId`. The supported profile also requires exactly one `meta` row with `key = '0'` under a unique-key schema: a duplicate or absent identity row is ambiguous, not proof. Missing, locked, malformed, unsupported, or mismatched stores reject the action before command construction, clipboard mutation, or terminal creation; they do not remove the metadata row. Detail decoding reuses the same profile/identity parser before following transcript roots. A store that is readable and claims a *different* agent identity also forecloses the project mirror — that contradiction is the stale/moved-directory signal the mirror must not paper over. An absent, locked, or unsupported store makes no competing claim, so the same-cwd same-id mirror still renders: treating capability drift as contradiction would blank previews the moment Cursor ships a newer schema, which is exactly what the mirror exists to survive.

The candidate is resolved once per action. `resolveCursorChatCandidate` enumerates every chat bucket, so resolving the entry and then independently re-resolving it for the proof both doubles that scan and lets the two resolutions observe different candidates across a move or delete-create. The launcher therefore takes one resolved launch target carrying both the mapped entry and its `dbPath`, and the proof runs against that same carried path.
