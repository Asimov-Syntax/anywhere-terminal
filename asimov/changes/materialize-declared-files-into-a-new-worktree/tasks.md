...
- [ ] 2_1 Bound the walk per operation, and make the production binding the one under test
  ...
  - **Plan**:
    ...
    3. `src/worktree/deadline.ts`, `src/worktree/deadline.test.ts`: a synchronously readable `expired` on `Deadline`. A deadline observable only through a promise is the defect's root — the walk's `.then` watcher cannot have run before the first node. `afterDelay` is its only implementor.
