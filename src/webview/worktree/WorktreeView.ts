      ...
      (actionResults ?? [])
        .map(
          (r) =>
            ...
            `${r.action}:${r.worktreeId ?? r.repoId ?? ""}:${r.orphanedLabel ?? ""}:${r.outcome}:${r.openFailed ?? ""}:${r.error ?? ""}${r.observed ?? ""}:${r.needsConfirm?.fingerprint ?? ""}:${provisionKey(r)}:${r.branchDelete ? `${r.branchDelete.kind}:${r.branchDelete.kind === "deleted" ? r.branchDelete.branch : r.branchDelete.reason}` : ""}`,
        )
        .join("|"),
  ...
  private buildActionNotice(result: WorktreeActionResult, info?: WorktreeInfo, name?: string): HTMLElement {
    ...
    if (result.outcome === "ok") {
      ...
      return renderNotice({
        ...
        reason:
          [result.openFailed, branchDeleteReason(result.branchDelete), brought?.reason, ported?.reason, setup?.reason]
            .filter((line) => line !== undefined)
            .join("\n") || undefined,
