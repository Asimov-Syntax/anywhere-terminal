  ...
  it("proves identity from a resolved session and carries its entry id", () => {
    expect(
      resolveAgentIdentity(
        input({ session: { kind: "resolved", agent: "claude", sessionId: "abc", evidence: "process" } }),
      ),
    ).toEqual({
      kind: "proven",
      agent: "claude",
    ...
    expect(
      resolveAgentIdentity(
        input({
          isAgentLaunch: true,
          shell: "claude",
          session: { kind: "resolved", agent: "claude", sessionId: "z", evidence: "process" },
        }),
      ),
...
describe("rank 4 — committed title", () => {
  ...
  it("loses to the session registry", () => {
    expect(
      resolveAgentIdentity(
        input({ title: "codex", session: { kind: "resolved", agent: "claude", sessionId: "q", evidence: "process" } }),
      ),
