...
export async function readRepoRefs(runner: GitCommandRunner, input: RepoRefsInput): Promise<RepoRefsRead> {
  const result = await runner.run(
    ["for-each-ref", "--format=%(refname:short)", `--count=${MAX_REFS + 1}`, "refs/heads/"],
    input.cwd,
  );
  ...
  const truncated = names.length > MAX_REFS;
  const holders = holdersByBranch(input.worktrees);
  const refs = names.slice(0, MAX_REFS).map((name) => {
    const heldBy = holders.get(name);
    return heldBy === undefined ? { name } : { name, heldBy };
  });
