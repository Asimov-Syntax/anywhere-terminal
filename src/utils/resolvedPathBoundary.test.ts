// Unit tests for ./resolvedPathBoundary — the containment predicate that
// resolves both sides before it answers. Split from pathBoundary.test.ts with
// the module itself; the assertions are unchanged.

import { describe, expect, it } from "vitest";
// The lexical predicate, imported to assert the CONTRAST between the two — the
// distinction the split must not blur.
import { isPathInside } from "./pathBoundary";
import { isResolvedPathInside, isResolvedPathInsideRoot, prepareResolvedRoot } from "./resolvedPathBoundary";
