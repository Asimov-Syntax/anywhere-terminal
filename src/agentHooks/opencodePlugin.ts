...
export const OPENCODE_PLUGIN_VERSION = 3;
...
// One id, not a set of them: the terminal is on one session at a time, and the
// only question this answers is whether the current one has been sent
...
async function report(sessionID) {
  ...
  if (!base || typeof sessionID !== "string" || sessionID === "" || reported === sessionID) return
  reported = sessionID
  try {
    ...
    // retry.
    if (reported === sessionID) reported = undefined
  }
}
