...
const SAMPLE: Record<(typeof WORKTREE_MESSAGE_TYPES)[number], Record<string, unknown>> = {
  ...
  worktreePickDestination: { type: "worktreePickDestination", repoId: "/repo/.git", token: 1, ask: 1 },
...
describe("the row-activation setting reaches the view", () => {
  ...
  it("carries no workbench rollout flag, and a stale one in settings changes nothing", async () => {
    // The rollout is over and the key is retired. A user whose settings.json
    // still turns it off must meet the workbench anyway — nothing reads the
    // value, so it can neither reach the webview nor fire a listener.
    __setConfigValues({ "anywhereTerminal.worktree.workbench": false });
    const view = await readyView();
    expect(view.posts("init")[0]).not.toHaveProperty("worktreeWorkbench");
    view.forget();
...
describe("a terminal request creates a pane in the surface that asked", () => {
  ...
  it("is implemented by a view surface", () => {
    ...
    const spy: WorktreeHost = {
      ...
      openingsHeld: () => host.openingsHeld(),
