...
| `worktreeCreate` | `{ repoId, opening, path, mode: WorktreeCreateMode, destination: DestinationDisposition, after: WorktreeAfterCreate, provision?: ProvisionSelection, migrateChanges?: boolean }` | Create a worktree. `mode` and `after` are **discriminated unions**, not flag sets — § 2.3 and § 2.6. `provision` names selections against a host-held offer and never carries command text (§ 2.4). `opening` names the form that composed this submit, and the host admits it only while that opening is the surface's live one — a form the user closed is not still asking for a worktree |
| `worktreeCreateClosed` | `{ opening }` | The form named by `opening` has closed — sent from **both** exits, cancel and submit alike, since neither told the host anything before. Retirement withdraws every channel that opening carries, not merely its offer: the destination read's marker, the provisioning offer, and the per-repository records behind the probe and the debris authorization. A result arriving for a retired opening mints no authority, publishes nothing, and leaves no state behind |
| `worktreeSetupViewOutput` | `{ outputId }` | Reveal the live setup terminal or its bounded replay. The opaque id is bound to the originating surface, worktree generation, and original directory authority; the webview never receives a path or transcript |
| `worktreeSetupRetry` | `{ worktreeId, retryId }` | Retry the retained setup steps only. The rotating opaque id is accepted only for the original surviving worktree identity; copy, link and ports are not repeated |
| `worktreeCreateProbe` | `{ repoId, token, seq, query: string, candidatePath?: string, base?: ProbeBase }` | Ask what a typed selection would resolve to — the whole selection, in every mode the form offers, a detached create included. `token` names the opening; `seq` is monotonic within it, because an A → B → A edit puts two answers on the wire identical in `token` and `query`. `candidatePath` carries a supplied destination as a CANDIDATE to assess, never as the path the create will take. `base` is what a new branch would start from, so the host can resolve it before submit. Answered by `worktreeCreateResolution` |
| `worktreeRemove` | `{ worktreeId, fingerprint?: string, deleteBranch?: BranchDeleteRequest }` | Remove a worktree. Without `fingerprint`, this is report intent and cannot reach git; with one, it is a confirmed attempt whose fresh host evidence chooses ordinary versus forced git execution. `deleteBranch` is absent unless the user opted in against a proof (§ 2.5) |
| `worktreeRemoveAssess` | `{ worktreeId, token }` | Ask for the removal report without acting. `token` orders answers and authorizes nothing. Answered by `worktreeRemoveAssessment`; these asks and fingerprint-free `worktreeRemove` intents share one bounded assessment lane per repository |
...
| `requestWorktreeCreateDefaults` | `{ repoId, opening, branch? }` | The destination this repo would use. Sent again whenever the branch settles, because the path is derived from it. The **branch-less** form is the only door that ESTABLISHES an opening; every other request rides one the surface already holds. A repeated ask naming the live opening joins the read in flight rather than starting another, and one naming an opening at or below the surface's high-water mark is answered with nothing, so a delayed or replayed ask cannot revive a form that has been superseded |
| `requestWorktreeRefs` | `{ repoId, token }` | The repository's local branches, for the create combobox. Its OWN message rather than a field on `requestWorktreeCreateDefaults`, which is re-asked per settled branch edit — the ref list does not change as the user types. `token` names the OPENING that is asking: `repoId` names a repository, so a dialog closed and reopened on the same one leaves two conversations whose answers are otherwise indistinguishable. It must EQUAL the surface's live opening — refs rides an opening, it never establishes or advances one, and this message is also the writer that seeds the per-repository record the probe and the debris authorization are gated on |
...
| `worktreeMutationResult` | `{ verb, repoId, worktreeId?, result }` where `result` is `{ kind: "ok", openFailed? }`, `{ kind: "error", message }`, `{ kind: "indeterminate", observed }`, `{ kind: "unavailable", unreadable }` or `{ kind: "blocked", worktreeId, fingerprint: string | null, assessment }` | Outcome of any mutating action, delivered to the SURFACE that started it. A fingerprint-free raw removal returns the `blocked` report rather than executing. `unavailable` is not a failure — nothing was attempted, because what the action would affect could not be read. `openFailed` rides on a success: the worktree exists and the window did not open |
| `worktreeCreateDefaults` | `{ repoId, opening, root, prefix, path, branch?, collidedWith? }` | The destination the create will actually use. `path` is free against BOTH the registry and the filesystem; `collidedWith` names the **last segment** of the unsuffixed candidate when it was taken — never a full path (§ 4.1). `branch` echoes the question, so a form can tell a current answer from one it has typed past. `opening` echoes the form that asked, and the panel drops a reply naming any other rather than caching or rendering it |
| `worktreeCreateResolution` | `{ repoId, token, seq, query, mode, freePath, occupiedCandidate?: { path, disposition }, blockedBy?: { ownerPath }, baseValid?: BaseVerdict }` | What the typed selection resolves to, echoing `token`, `seq` and `query` for staleness. `freePath` is always present — every mode has a destination, and it is the path the form states and submits. `baseValid` is the verdict on `base`, withheld for the modes that take no base. The answer authorizes nothing on its own: the host re-identifies the opening by `(surface, repository, token)` after every await, so a probe suspended while its repository leaves the workspace posts nothing. `occupiedCandidate` is the path the suffixing skipped and what was found there — without it, debris is invisible and recover is unreachable. `blockedBy` marks a branch checked out elsewhere: offered disabled, never submittable |
| `worktreeRefs` | `{ repoId, token, refs: WorktreeRef[], truncated }` | The repository's local branches. `token` echoes the request, and an answer below the current opening's is dropped. Each ref carries `heldBy`, the NAME of the directory whose worktree holds it — never a path, on the same rule `collidedWith` follows. Distinct from § 2.2's `worktreeCreateResolution.blockedBy`, which carries `{ ownerPath }` for the one typed selection: this message answers a LIST, where every row needs its own state at once. `truncated` says the enumeration hit its cap, so a partial list is stated as partial rather than presented as the repository's complete set. Absent is "not told yet", NOT "there are none" |
| `worktreeProvisionOffer` | `{ repoId, opening, model: ProvisionModel, offerId, expiresAt }` | The provision model the dialog displays, and the id a create must cite (§ 2.4). `opening` echoes the form that asked, on the same rule as `worktreeCreateDefaults` |
| `worktreeRemoveAssessment` | `{ worktreeId, token, result }` where `result` is `{ kind: "assessed", assessment: { checks: RemovalCheck[], contained }, fingerprint: string | null, branchDelete?: BranchDeleteOffer }` or `{ kind: "unavailable", unreadable }` | The removal report (§ 2.5). `token` echoes the ask; unavailable is distinct from a refusal, and only a readable non-refused assessment carries confirmation authority |
| `worktreeProvisionResult` | Initial: `{ worktreeId, steps, ports, contests?, setup?, setupOutputId?, setupRetryId?, manifestWarning? }`; setup-only update: `{ worktreeId, setup, setupOutputId?, setupRetryId?, manifestWarning? }` | Initial provisioning arrives after the create result and never changes whether create succeeded. A retry replaces setup state only; it structurally cannot replay material, ports, or contest membership. Output and retry capabilities are opaque and disappear when superseded |
...
**Retirement.** An offer belongs to the opening that asked for it. When that form closes, the
offer is evicted with everything else the opening carried (§ 2.1, `worktreeCreateClosed`) — the
webview cannot submit against it because the form is gone, and the host will not honour it because
the record is gone. This is a separate mechanism from the refresh rule below, which governs an
offer whose form is still open.
...
- [ ] Fingerprint-free `worktreeRemove` on a clean or dirty worktree → blocked report with a fingerprint, no git run
- [ ] Re-sent with the matching fingerprint while still dirty → forced git removal; while now clean → ordinary git removal
- [ ] Re-sent after a live agent appeared → refused or re-prompted, git never runs
- [ ] Re-sent with a stale or unknown fingerprint → re-prompts rather than authorizing
- [ ] Mixed `worktreeRemoveAssess` and fingerprint-free `worktreeRemove` bursts → at most one assessment job per repository ahead of a confirmed mutation
...
- [ ] `worktreeRemove` on the main worktree → refused, `isMain: true`
