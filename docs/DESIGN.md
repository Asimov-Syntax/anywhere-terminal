...
| Provisioning manifest | `.git/worktrees/<id>/anywhere-terminal-provision.json` — a **record, not an authority**. Never decides what to delete; a missing one degrades a claim to could-not-be-determined | [worktree-apply.md](design/worktree-apply.md) § 2.6 | removal § 2.3 |
...
| Setup gate | Copy, link and ports always complete before `openAfter`. Setup may run concurrently with it, or be sequenced ahead of an agent start by an explicit per-create choice | [worktree-create.md](design/worktree-create.md) § 6 | provisioning § 5.4, rpc § 2.6 |
