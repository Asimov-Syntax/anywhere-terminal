...
[worktree-create.md](design/worktree-create.md) ·
[worktree-removal.md](design/worktree-removal.md) ·
[worktree-provisioning.md](design/worktree-provisioning.md) ·
[worktree-apply.md](design/worktree-apply.md) ·
...
| Shipped bundle | A relative `require` surviving into `dist/extension.js` that will not resolve beside it fails the build, not the user's activation. `pnpm run build:check-requires` reads the built artifact — the suite resolves dependencies' ESM entries and never loads the bundle, so no test can see this class. One completeness claim, for the relative class only: every relative string literal in the artifact is swept. An absolute literal naming the build root warns; a bare specifier is not reported, because telling one from ordinary text needs type information the gate deliberately does not build |
...
| D31 | Containment is resolved, not spelled — every vault resolver realpaths both sides | Keep lexical `path.relative` containment; or fix only the resolver a review happened to open | A symlink inside the root satisfies every lexical test while resolving outside it, and the repo already states the stricter rule for webview-supplied paths (§ 8.5). Tolerance is for **absence only** — the existing `realpathTolerant` swallows every error and rebuilds lexically, which would let a dangling link through, so it stays an availability helper and is not reused here. Two rules follow from the answer authorizing a READ: component case stays significant (both sides are post-`realpath`, so a surviving difference is a real one, and folding could only erase it), and a file reached by ENUMERATING the root is checked like one reached by resolving an id. One rule at every site; fixing one site alone makes it the odd one out |
| D32 | A window's "first row-drawing surface" is defined once and every boundary routes through it | Add the missing branches at each site that decides it inline | The concept decides whether presence is rebuilt with row **enrichment** — subscription is a separate question with its own predicate. Spelled inline it had drifted at two boundaries; the defect is the absence of an owner, so adding branches would reproduce it. What is owned is the conjunction — rows drawn AND the published envelope built bare — reconciled after every change to its inputs, plus a rebuild that cannot finish while it holds |
| D33 | A transcript look is time-bounded and **fails soft** — a timeout means "achieved nothing", and the attempt that timed out commits nothing it later finds | No timeout; or surface a read timeout as an error state on the row | The cadence gate bounds how often a look starts, never how long one takes, so a hung mount holds its slot forever. Failing soft feeds the existing retry ladder: a slow transcript is usually readable later, and a row that blanks on a sleeping volume tells a worse lie than one keeping its last known line. "Commits nothing" is the half that is not free: a look writes through the entry it was given, so an attempt abandoned at its deadline would otherwise go on to blank the very line the deadline preserved. It writes a draft the winner of the race commits instead |
| D34 | A preview is suppressed only on exact equality with the title, after the title's OWN normalization — the preview is never normalized | Fuzzy or prefix similarity; or leave the duplicate line | Every session is a one-message session at first render, so the row draws one sentence twice. But a near-match still carries something the title did not — a heuristic that hid it would replace a redundancy with a worse lie. Normalizing both sides would read more naturally and is forbidden: the title's stripper eats a leading `- ` or `*`, which in message text is a marker the model wrote (worktree-agent-presence § "A preview is message text, not a pane title") |
| D35 | The preview service owns "this entry is gone", as a third outcome beside unresolved and uncovered | Push the projector's live entry-id set down into the service | The service already re-resolves on cadence and already distinguishes "not there yet" from "never will be". Naming deletion there keeps the knowledge where the syscall happens, with no cross-layer push and no second definition of "live" to keep in sync. A temporarily unresponsive transcript is explicitly NOT a deletion — but a read that fails outright still retires the line (D33 grants the kept line to a timeout only). Requires a conclusive absence signal from the vault: `getEntry` returning `null` today means *unknown*, not *absent* |
| D36 | A vault entry lookup answers `found` \| `absent` \| `unknown`; only `absent` may retire a preview | Keep the `null` return and treat it as deletion; or corroborate `null` with the transcript file's absence | `getEntry` collapses a deleted entry, an unlocatable session path, an unparseable file and a caught reader error into one `null` — `codexReader.ts` documents it as `// query-error → unresolved (caller treats null as unknown-entry)`. Building deletion on `null` retires a live session's line whenever SQLite hiccups. Corroborating with the file does not work either: the entry-gone-but-file-present case is the actual debt. The readers already hold the distinction — an empty result set is absent, a thrown query is unknown — so this surfaces what they know rather than adding a probe |
| D37 | Attribution resolves the **bounded** side of a comparison once, where that set is produced, and leaves the per-event side lexical | Resolve at each comparison site; or resolve everything eagerly at startup | These comparisons run per presence push and per git refresh, so a `realpath` per comparison is the cost the feature cannot pay, and an eager sweep over paths the user never asked about triggers macOS TCC prompts. The distinction from D31 is what the answer AUTHORIZES: D31 gates a READ and therefore forbids caching a resolved candidate, while this one decides which heading a row appears under and therefore requires it. The resolved value is cached per distinct spelling and held by its CLAIMANTS — released when the last consumer lets go, which is not the same event as a structural invalidation: a pane closing means one consumer stopped asking, not that the directory moved. A consumer that reads immediately rather than standing on the answer claims per transaction |
...
| D37 | Provisioning reads a **normalized model merged from several providers** — asimov, orca, VS Code tasks, and a native file — rather than one config format | Define our own format and require repos to adopt it; or read only `asimov/worktree.yaml` | The material a fresh worktree needs is already declared in most repos that need it, in a file some other tool owns. Inventing a format makes every existing repo unconfigured on day one. Reading several means the section is populated for an asimov repo and an orca repo without either being asked to change |
| D38 | `.vscode/worktree.json` merges **additively** over what it `extends`, deduped by path with the inline entry winning, plus an `exclude` list; provenance is per entry | Per-key replacement; or forbid mixing `extends` with inline keys | The UI draws a source badge per row, so per-key replacement would collapse provenance to one badge per section and discard exactly the information the section exists to carry. Additive keeps every row able to name the file that declared it; `exclude` is what makes subtraction possible without copying the framework's whole list out |
| D39 | `runOn: "worktreeCreated"` is honoured as a **convention read from `tasks.json` directly**, never through the API | Depend on the VS Code task API to surface it; or invent our own setting | The schema key is real, but `TaskRunOn` lives in `vscode.proposed.taskRunOptions.d.ts` and a published extension cannot enable a proposed API. Its dispatcher is core-internal to Agent Sessions and never fires for a worktree a third-party extension created. Parsing the file honours the same key with the same meaning, depends on nothing unreleased, and is what gets deleted if the proposal ships |
| D40 | **The extension allocates ports under a cross-process lock** in the repository's common git directory, excluding values any sibling worktree's `.env.worktree` already claims | Display only what the owning tool allocated; show names without numbers; or scan siblings without a lock | Port collision is the research's #3 ranked pain precisely because the numbers are invisible, and a row naming ports without values does not pay that off. A scan alone does not deliver it: two windows can read the same sibling set and probe independently, so the lock is what makes "never twice" true. The claim is deliberately bounded — it covers worktrees this extension creates, and an unrelated process can still bind the port before setup runs. The preview/applied split is stated rather than hidden: the user was shown a number in order to remember it |
| D41 | Fresh / fresh-detached / reuse / reattach / adopt are **first-class branch modes of the selection**, resolved before submit, with recover as an independent destination disposition; the base ref is contractual and refused where it cannot apply | Leave them all as git failures after submit; or make reuse first-class and leave the rest as errors | Each is a way a branch or a destination is already occupied, and today each arrives as a git failure *after* the user committed to the action. Resolving them at selection time is what turns "git refused" into a choice. Refusing the base rather than ignoring it follows spec-kitty: a field that will not be applied must not sit in the form looking as though it will |
| D42 | No branch-name generation | Use VS Code's `generateRandomBranchName()` as a placeholder | This form makes the branch name its lead input and blocks submission until it validates. Generated names fit a product whose subject is a workspace that happens to have a branch, which is orca's shape, not this one. A wrong-but-plausible name is worse than a blank field |
| D43 | Removal is a **report**, and what "unproven" costs depends on the check's class: a hard refusal unproven still refuses, a confirmable risk unproven still confirms, and a proof unproven withholds only the option it gated. Typed confirmation is earned by a confirmable risk, never by a withheld proof | Show only the problems; or require typed confirmation on every removal | A list of problems alone gives no way to judge how much was looked at. A single "unproven blocks" rule was tried and is not implementable: it would let an unfetched default branch prevent a removal nobody asked to delete a branch for. Fail-closed still holds within each class, and matters because the cheap "is this branch merged?" returns false for both *not merged* and *could not tell*. Typing on every removal trains the user to type past it, which is the opposite of the intent |
| D44 | Branch deletion becomes an **opt-in, proof-gated, two-OID-guarded** step after a successful removal — **reverses** the earlier "never part of removal" rule | Keep branch deletion out of removal entirely; or delete the branch whenever the worktree goes | The earlier reasoning — that bundling it would destroy work the user believed was merely un-checked-out — is preserved in full; what changes is that *silently* was carrying that sentence. Off by default, offered only on a proven merge, and guarded, it is not the thing that reasoning refused. Both OIDs are guarded, not just the branch's: the merge proof is a claim about two refs, and guarding one would let the other move under a proof no longer true. The result is **guarded and fail-closed**, not *provably safe* — another process can still check the branch out or advance it in the window a ref transaction does not cover, and the delete fails rather than proceeding on stale evidence |
| D45 | Orphan proofs (lock age, dead owning PID, merged branch) are **computed and displayed, never auto-reaped** | Reap orphans automatically when all three hold; or skip the proofs | The proofs make a human decision better-informed. An automatic delete path justified by three heuristics is a new way to lose work, in the one area of this extension where mistakes are unrecoverable |
| D47 | A surviving checkout whose administrative entry was deleted is **adopted by reconstructing the entry in place** — write `gitdir` / `commondir` / `HEAD` and the worktree `.git`, then `git worktree repair`, then `git -C <path> reset --mixed` | Move the directory aside, `git worktree add --no-checkout` into the freed path, move the contents back, reset; or do not offer adoption at all and degrade to reuse at a fresh path | No git command attaches a populated directory: `repair` requires the administrative directory to already exist, and `add` refuses a non-empty destination before `--force` is consulted. Both verified against git 2.50.1 and against upstream source; upstream discussed a `--keep-worktree` adoption flag in 2019 and never implemented it. Move-aside reaches the same state through supported commands and inherits git's already-checked-out guard, but relocates every file twice — slow over a large dependency tree, a copy across filesystems, and an interruption leaves content in two places, manufacturing exactly the debris state recover exists to clean up. Reconstruction writes four small files, touches nothing inside the working tree, and is undone by deleting the entry. Its one cost is that it bypasses git's guard, which becomes an explicit host-side refusal (D48) |
| D48 | Adopt **refuses outright** when any live worktree already holds the branch, checked against `git worktree list --porcelain` before any file is written | Warn and let the user proceed; or rely on git to catch it | Git makes this check inside `worktree add` and cannot make it for a reconstructed entry. Verified: two entries claiming one branch produce no message at all, and the second commit lands on top of the first carrying a tree that silently reverts it — no conflict, no warning, a linear history. A warning is the wrong register for a failure the user cannot detect afterwards |
| D46 | Create and removal each move to their own design doc; `worktree-actions.md` becomes the hub, and `worktree-rpc.md` is the standing companion every task reads alongside its owner | Keep one actions document and let it grow | The redesign would have taken `worktree-actions.md` past 700 lines, and a downstream agent reads one design doc rather than the whole subsystem. Splitting on the two mutating actions that own real safety models keeps each readable and leaves the shared rules in one place. The wire contract is the one exception to "one doc": behaviour and message shape cannot both live in the owner without duplicating one of them, so `worktree-rpc.md` is named as a companion rather than pretended away |
...
2026-08-29, against the audit in `docs/audit/2026-08-29-worktree-ui-vs-orca.md`. D37–D39 and D46
were recorded rather than asked; D40–D45 were chosen by the user at the blueprint gate on
2026-08-30, against `docs/audit/2026-08-30-worktree-lifecycle-gaps.md`. **D44 reverses a rule
recorded in the superseded `worktree-actions.md` § 3.3** and is the one decision here that
contradicts an earlier one rather than extending it.
...
| Worktree settings keys | `anywhereTerminal.worktree.createRoot` (string, default `.claude/worktrees`; relative resolves against the main worktree, absolute used as-is; set explicitly it outranks detection, unset the repo's own layout wins), `anywhereTerminal.worktree.rowActivation` (`focus` \| `preview`, default `focus`) | [worktree-create.md](design/worktree-create.md) § 3, [worktree-panel-ui.md](design/worktree-panel-ui.md) § 6 | — |
| `WorktreeOpenAfter` | `none` / `terminal` / `agent` / `newWindow` / `addToWorkspace` | [worktree-rpc.md](design/worktree-rpc.md) § 2.2 | create § 4 |
...
| Vault path containment | Resolved on both sides, then a strict relative test with component case preserved (only separators and the drive letter fold). Only an absent tail beneath a resolved in-root parent is tolerated; any other resolution failure refuses. The root resolves once per operation, the candidate on every check, and no answer is cached. One rule at every vault resolver, enumerated entries included | [worktree-subsystem-debts.md](design/worktree-subsystem-debts.md) § 2.1 | § 8.5, § 9 D31 |
| Worktree and repository attribution | Resolved once per distinct spelling on the side a bounded producer owns — panes held, folders open, repositories listed, the root a tree mounts — and lexical on the side that arrives per event. One containment rule, shared with the webview; the resolved workspace root reaches the webview as its own field so the mounted root keeps the user's spelling | [worktree-subsystem-debts.md](design/worktree-subsystem-debts.md) § 2.1 | § 9 D31, D37 |
| Row-drawing promotion | The conjunction — a surface is drawing rows AND the published presence envelope was built without enrichment. Reconciled after every change to a surface's visibility, level or displayed state; a rebuild cannot finish while it holds. Distinct from the predicate that arms the scan | [worktree-subsystem-debts.md](design/worktree-subsystem-debts.md) § 2.2 | § 9 D32 |
| A failed transcript look | Three outcomes, never conflated: `unresolved` (not there yet — retried), `uncovered` (this source keeps no transcript — never retried), and a vanished vault entry (retires the line, and is re-confirmed on its own interval rather than being final). A timeout is `unresolved`, not a fourth; a read that fails outright retires the line on the ladder. "Vanished" requires a conclusive absence signal — never inferred from a `null` lookup, and an inconclusive one changes nothing at all: not the line, not the confirmation time, and no filesystem work | [worktree-subsystem-debts.md](design/worktree-subsystem-debts.md) § 2.3, § 2.5 | § 9 D33, D35 |
| Outstanding transcript work | One look per session at a time, and never more in flight than the preview retains. Tracked outside the entry cache, so eviction cannot release a session whose look is still stalled | [worktree-subsystem-debts.md](design/worktree-subsystem-debts.md) § 2.3 | § 9 D33 |
...
| Provision model | `ProvisionEntry { path, mode: "copy" \| "link", source }`, `ProvisionSetupStep { kind: "shell", script, source }` — one variant, because a task cannot be run for a directory that is not a workspace folder — `ProvisionPort { name, source, port }` — `source` is **never absent**, on any entry, after any merge | [worktree-provisioning.md](design/worktree-provisioning.md) § 2 | create § 4.3 |
| Provider detection order | `.vscode/worktree.json` → `asimov/worktree.yaml` → `orca.yaml` / `.worktreeinclude` → `.vscode/tasks.json`; **first hit supplies the base**, later hits are recorded `active: false` and offered, never merged | [worktree-provisioning.md](design/worktree-provisioning.md) § 4.1 | create § 4.3 |
| Provisioning merge rule | Additive; dedupe by `path` with the inline entry winning including its `mode`; `exclude` moves an inherited entry to `excluded` keeping its original `source`; `setup` steps are never deduped or reordered | [worktree-provisioning.md](design/worktree-provisioning.md) § 4.2 | create § 4.3 |
| Provisioning apply order | `copy → link → ports → setup` — fixed. Ports precede setup because setup consumes them; setup is last because it consumes copied config | [worktree-apply.md](design/worktree-apply.md) § 1 | create § 6 |
| Native provisioning file | `.vscode/worktree.json` — the **only** file the extension writes; a provider file is never rewritten | [worktree-provisioning.md](design/worktree-provisioning.md) § 6 | create § 4.3 |
| Per-worktree port file | `.env.worktree`, `<NAME>=<port>` lines, written into the new worktree and read from siblings to exclude claimed values. Never overwritten if it already exists | [worktree-apply.md](design/worktree-apply.md) § 2.3 | — |
| Create modes | `fresh` / `fresh-detached` / `reuse` / `reattach` / `adopt` — branch modes of the **selection**, resolved before submit. A branch checked out elsewhere is not a mode: it is offered disabled, badged with its owner. Recover is not a mode either: it is a destination disposition | [worktree-create.md](design/worktree-create.md) § 2 | rpc § 4 |
| Removal check outcome | `passed` / `failed` / `unproven` / `notApplicable` — **unproven blocks** and is never rendered as passed; `notApplicable` never blocks and is never rendered as unproven (a detached HEAD has no branch to prove merged). Typed confirmation is present only when at least one check is `failed` or `unproven` | [worktree-removal.md](design/worktree-removal.md) § 2 | ui § 5, rpc § 2.5 |
| Collision statement | The suffixed **result segment only**, never a path. The wire field and its rendering agree on which side shortens | [worktree-create.md](design/worktree-create.md) § 4.2 | rpc § 2 |
| Create mode vs destination | Branch mode (`fresh` / `fresh-detached` / `reuse` / `reattach` / `adopt`) and destination disposition (`free` / `debris`) are **independent** and travel as two values. Recover is a disposition, not a mode, and does not disable the base ref | [worktree-create.md](design/worktree-create.md) § 2.0 | rpc § 2.3 |
| Reattach precondition | Git's own `prunable` flag — the administrative entry **survives** and its path is stale, so `git worktree repair` fixes it and nothing is lost. Verified against git 2.50.1 | [worktree-create.md](design/worktree-create.md) § 2.3 | model § 2, rpc § 2.3 |
| Adopt precondition | The administrative entry is **gone** and a populated checkout survives. Neither `repair` nor `add` can attach it, so the entry is reconstructed; the branch must be held by no live worktree, and the index, in-progress operations, per-worktree refs, `config.worktree` and lock state are **declared lost**, never probed. Verified against git 2.50.1 | [worktree-create.md](design/worktree-create.md) § 2.4 | rpc § 2.3 |
| Provisioning offer | Host-held model + opaque `offerId`; the webview submits `itemIds` only. An unknown or invalidated offer performs no work, re-presents, and requires a second submission | [worktree-provisioning.md](design/worktree-provisioning.md) § 4.0 | rpc § 2.4, create § 4.3 |
| Provisioning manifest | Every successful fresh create atomically writes `.git/worktrees/<id>/anywhere-terminal-provision.json`, including an empty record for an empty selection — a **record, not an authority**. Never authorizes setup, retry, or deletion; a missing one degrades a claim to could-not-be-determined | [worktree-apply.md](design/worktree-apply.md) § 2.6 | removal § 2.3 |
| Port allocation scope | Distinctness is guaranteed **only across worktrees this extension creates**, under a lock in the repository's common git directory. An unrelated process may still bind the port | [worktree-apply.md](design/worktree-apply.md) § 2.3 | — |
| Setup gate | Copy, link and ports complete first. Without the wait choice, agent launch overlaps the setup run; with it, an agent starts only after every selected step succeeds. Failure leaves the worktree standing with opaque output/retry actions on its row | [worktree-create.md](design/worktree-create.md) § 6 | apply § 2.4–2.5, rpc § 2.6 |
| Branch-delete guard | Both ref **names** and both OIDs — branch and default branch — plus the assessment fingerprint. Guarded and fail-closed, never "provably safe" | [worktree-removal.md](design/worktree-removal.md) § 5 | rpc § 2.5 |
