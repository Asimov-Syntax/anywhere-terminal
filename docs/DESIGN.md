...
| A failed transcript look | Three outcomes, never conflated: `unresolved` (not there yet — retried), `uncovered` (this source keeps no transcript — never retried), and a vanished vault entry (retires the line permanently). A timeout is `unresolved`, not a fourth; a read that fails outright retires the line on the ladder. "Vanished" requires a conclusive absence signal — never inferred from a `null` lookup | [worktree-subsystem-debts.md](design/worktree-subsystem-debts.md) § 2.3, § 2.5 | § 9 D33, D35 |
