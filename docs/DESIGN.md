...
| Provision model | `ProvisionEntry { path, mode: "copy" \| "link", source }`, `ProvisionSetupStep { command, source }`, `ProvisionPort { name, source, port }` — `source` is **never absent**, on any entry, after any merge | [worktree-provisioning.md](design/worktree-provisioning.md) § 2 | create § 4.3 |
...
| Provisioning apply order | `copy → link → ports → setup` — fixed. Ports precede setup because setup consumes them; setup is last because it consumes copied config | [worktree-provisioning.md](design/worktree-provisioning.md) § 5 | create § 6 |
...
| Per-worktree port file | `.env.worktree`, `<NAME>=<port>` lines, written into the new worktree and read from siblings to exclude claimed values. Never overwritten if it already exists | [worktree-provisioning.md](design/worktree-provisioning.md) § 5.3 | — |
...
| Provisioning manifest | `.git/worktrees/<id>/anywhere-terminal-provision.json` — a **record, not an authority**. Never decides what to delete; a missing one degrades a claim to could-not-be-determined | [worktree-provisioning.md](design/worktree-provisioning.md) § 5.6 | removal § 2.3 |
| Port allocation scope | Distinctness is guaranteed **only across worktrees this extension creates**, under a lock in the repository's common git directory. An unrelated process may still bind the port | [worktree-provisioning.md](design/worktree-provisioning.md) § 5.3 | — |
