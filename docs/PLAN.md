...
| **Acceptance** | Two declarations whose normalized paths are exactly equal produce one entry, and the native one wins including its `mode` while keeping its own `source`; two declarations related only by case or Unicode folding stay a pair that is drawn with both spellings and both sources, carries no checkbox, and is never redeemed as two independent destinations; `exclude` matches on the same rule the merge uses, and an exclusion matching nothing is reported rather than dropped; no identity decision reads any path at all, inside the repository root or outside it; the read path still imports nothing that executes or mutates |
| **Status** | todo |
