...
| **Size** | S |
| **Labels** | None |
| **Notes** | A **spike**, and it gates WT-012.15. The reconstruction recipe was verified only on macOS with git 2.50.1. Every one of its four files carries a path, one of them absolute, and Windows differs on separator, drive letter, case sensitivity and what `git worktree repair` normalises. The recipe writes into git's own administrative directory, so a platform where it half-works is worse than one where it does not work at all — the failure has to be established before the feature is built on top of it. **NO LONGER BLOCKED ON OWNING A WINDOWS MACHINE**, and the earlier "needs a Windows host" note was wrong: orca runs exactly this shape as a GitHub-hosted CI job — `orca/.github/workflows/daemon-relocation-spike.yml` is a 97-line spike workflow on `windows-2022`, no secrets, no self-hosted or paid runner, opening with a `# Why:` block stating the empirical question, ending in an `if: always()` artifact upload of a log whose last line is `VERDICT: PASS|FAIL`. Two gotchas it records: `workflow_dispatch` does not appear until the file is on the default branch, so a feature-branch `push` trigger is the workaround; and `windows-latest` is right for a spike that compiles nothing, while `windows-2022` is pinned only where node-gyp must detect Visual Studio. **Most of this spike does not need Windows at all**: orca settles separator and drive-letter casing from macOS by taking `platform` as a defaulted parameter and using `node:path/win32` (`orca/src/main/ipc/worktree-path-comparison.ts:4-8,102-104`), and keeps the Windows-only execution behind a thin shim whose decision functions are unit-tested anywhere. Only real filesystem case-insensitivity and `git worktree repair`'s own rewriting need the runner. A **fourth axis** this row never listed and orca does test: Unicode NFD vs NFC (`orca/src/main/runtime/worktree-rm-id-selector-path-spelling.test.ts:48-58`). Vary `core.autocrlf` per subprocess via `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_0` rather than mutating the runner; use a junction instead of a symlink, since ordinary Windows CI tokens cannot create file symlinks without Developer Mode. Long paths have NO workaround in orca — if the reconstruction produces deep paths that is unowned. Finally: orca reads the same four admin files but **never invokes `git worktree repair`**, so there is no prior art for this row's actual question. **This row now owns a second deliverable**: WT-012.15 ships `adoptSupported`, a predicate that withholds adopt on win32 while the recipe is unrun, and this row supplies its value — a PASS sets the default true, a FAIL leaves it false and replaces the reason with the captured failure. "Unverified" is the correct wording only while the recipe has not run, which is why the flip is owned here rather than left for whoever notices |
...
| Field | Value |
|-------|-------|
| **Goal** | Close the redirections a pure-Node runtime CAN close on the paths this extension writes — a staging name nothing can be waiting at, and a release that never removes a lock it cannot prove is its own — and record the ones it cannot as accepted risks a user can act on. Narrowed from "operate on the directory that was authorized", which a plan attack showed no mechanism here reaches: Node exposes no `*at`, and the descriptor-validation tier the Notes proposed buys nothing where the open is already `O_CREAT\|O_EXCL` |
| **Design Ref** | [worktree-provisioning.md](design/worktree-provisioning.md) § 7 |
| **Depends On** | WT-012.19 |
| **Stage** | 10 |
| **Size** | L |
| **Labels** | security-privacy |
| **Notes** | The part of the original WT-012.19 that a plan attack showed no pure-Node mechanism can deliver, kept visible rather than closed by silence. Two acceptance clauses were abandoned there and are restated here: the four operations still NAME STRINGS, so a held descriptor is only an identity oracle and two writers resolving one spelling can hold two live locks; and cleanup reaches the temporary through the current spelling, so a directory renamed under a part-way failure keeps it. Detection has an irreducible ABA hole this task exists to close: redirect after a checkpoint, let an unguarded operation land on the decoy, restore before the next checkpoint, and every comparison compares equal. The only mechanisms that close it fully are a native addon binding `openat`/`renameat` — prebuilt binaries per platform, architecture and Electron ABI for an extension whose three dependencies are all pure JS — or accepting the residual as a stated limitation. **The reference implementations have now answered this and the row is no longer waiting on a decision.** Neither orca nor cmux ships an N-API addon to an Electron host: cmux distributes a standalone prebuilt executable behind a spawn shim over five `platform-arch` targets with NO Node ABI dimension at all, and orca's `native/` holds out-of-process helper programs (Swift, C#, PowerShell, Python), never addons. Where orca does consume third-party addons it pays by rebuilding from source per runtime, which is why its ABI axis is a literal `--runtime=node|electron` flag. An in-process addon here would carry platform × arch × (Node ABI **and** Electron ABI), and VS Code's Electron moves independently of ours, so that last axis re-breaks on host upgrades. **The prior belief that cmux avoids `*at` was half wrong and the correction is the useful part**: cmux uses `openat`/`mkdirat`/`fstatat`/`unlinkat`/`renameat2` heavily for WORKSPACE FILE MUTATION (`cmux/cmux-tui/crates/chatmux-relay/src/workspace.rs`), and deliberately does NOT anchor its LOCKS — every lock site opens an absolute path with `O_CREAT \| O_RDWR \| O_NOFOLLOW` (`cmux/CLI/cmux_open.swift:3298`, and five more) and then **validates the open DESCRIPTOR rather than the path** — `fstat` for `S_IFREG`, uid, `nlink == 1`, `mode & 0o077 == 0` — before `flock` (`cmux/CLI/CMUXCLI+PiExtension.swift:45-58`). That middle tier is reachable in pure Node through `fstat` on the open handle and closes strictly more than nothing, which makes this row deliverable work rather than a coin flip. What the residual limitation must then SAY has a model to copy — `cmux/cmux-tui/spec/transports.md:128-135` states scope covered, the exact mechanism that leaks, the named syscall it is declining to use marked "future", and the threat condition under which it matters, with no hedging. Note both projects keep such statements in source comments and spec files, never in a README. **Shipped**: a staging name of 16 random bytes created `wx` — `CursorHookInstaller` named it from the clock and created it with `writeFile`, which FOLLOWS a symlink, so a guesser had a write into any file the host's user can write; Cursor's duplicate lock deleted in favour of `LockedFile`, whose release refuses a name it cannot prove is its own; a `createParent` option so delegation stops creating a Cursor configuration directory for a user who may not have Cursor; and R1-R4 stated in the design doc with triggers and remedies. **Not shipped, and no longer this row's**: anchoring the four operations to a directory object. The descriptor-validation tier these Notes proposed was REJECTED on evidence — cmux opens its lock without `O_EXCL` and so can land on a pre-existing object worth inspecting, while `wx` here refuses every pre-existing object before any `fstat` runs, which is why all three witnesses written against it passed on `EEXIST` alone. Three review rounds; the two blockers of round 1 and the two of round 2 were all defects introduced by this change's own fixes, and each was accepted rather than rebutted |
